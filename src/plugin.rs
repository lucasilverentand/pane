...
//!
//! ## Protocol
//! Input (to plugin stdin):
//!   `{"event":"workspace_change","workspace":"name","system_stats":{"cpu_percent":42.5,...}}`
//!
//! Output (from plugin stdout):
//!   `{"segments":[{"text":"  main","style":"accent"}],"commands":["select-pane -L"]}`
...
impl PluginManager {
    ...
    /// Start all plugins. Call once after creation.
            ...
            Ok(mut child) => {
                ...
                if interval > 0 {
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            ticker.tick().await;
                            // The actual send happens via send_event; this just triggers a "refresh" event
                            let _ = event_tx.send(PluginEvent::Commands {
                                commands: vec![], // empty commands = just a refresh trigger
                            });
                        }
                    });
                }
    ...
    /// Send an event to all plugins that subscribe to it.
    pub async fn send_event(
        &mut self,
        event: &str,
        workspace: Option<&str>,
        stats: Option<&crate::system_stats::SystemStats>,
    ) {
    ...
    /// Get all current plugin segments for rendering.
