...
/// Create a widget window snapshot.
...
/// Create a home workspace with widget windows.
...
fn home_workspace_basic() {
    ...
    let output = render_to_string(&mut client, COLS, ROWS);
    insta::assert_snapshot!("home_workspace", output);
}
...
// ---------------------------------------------------------------------------
// Home workspace focus tests
// ---------------------------------------------------------------------------
//
// These tests use `render_to_styled_string` which replaces active (bright)
// border characters with heavy box drawing (┏━┃┗┛┓) while inactive borders
// stay as thin rounded (╭─│╰╯╮). This makes focus state visible in text.
//
// Legend:
//   ┏━┓  = active (focused) border
//   ╭─╮  = inactive border

/// Home workspace: sidebar focused, no widget focused.
/// All widget borders should be inactive (thin).
#[test]
fn home_sidebar_focused_all_widgets_inactive() {
    let mut client = Client::for_test(Config::default());
    let (w1, w2) = (new_id(), new_id());
    let (t1, t2) = (new_id(), new_id());

    let ws = home_workspace(
        vec![
            widget_window(w1, t1, HubWidget::RecentCommits),
            widget_window(w2, t2, HubWidget::Branches),
        ],
        hsplit(LayoutNode::Leaf(w1), LayoutNode::Leaf(w2)),
    );
    ...
    // Sidebar is focused: no focused_widget, focus is on WindowLayout
    client.project_hub_state = Some(ProjectHubState::for_test());

    let output = render_to_styled_string(&mut client, COLS, ROWS);
    insta::assert_snapshot!("home_sidebar_focused_all_inactive", output);
}

/// Home workspace: first widget focused.
/// Only the focused widget should have active (heavy) borders.
#[test]
fn home_widget_focused() {
    let mut client = Client::for_test(Config::default());
    let (w1, w2) = (new_id(), new_id());
    let (t1, t2) = (new_id(), new_id());
...
/// Home workspace: second widget focused.
/// Only the second widget should have active borders.
#[test]
fn home_second_widget_focused() {
    let mut client = Client::for_test(Config::default());
    let (w1, w2) = (new_id(), new_id());
    let (t1, t2) = (new_id(), new_id());
...
/// Home workspace: workspace bar focused.
/// All widget borders should be inactive.
#[test]
fn home_workspace_bar_focused() {
    let mut client = Client::for_test(Config::default());
    let (w1, w2) = (new_id(), new_id());
    let (t1, t2) = (new_id(), new_id());
...
/// Home workspace with three widgets in a complex layout.
/// Only the focused one should have active borders.
#[test]
fn home_three_widgets_middle_focused() {
    let mut client = Client::for_test(Config::default());
    let (w1, w2, w3) = (new_id(), new_id(), new_id());
    let (t1, t2, t3) = (new_id(), new_id(), new_id());
    ...
    let ws = home_workspace(
        vec![
            widget_window(w1, t1, HubWidget::RecentCommits),
            widget_window(w2, t2, HubWidget::ChangedFiles),
            widget_window(w3, t3, HubWidget::Branches),
        ],
        hsplit(
            LayoutNode::Leaf(w1),
            vsplit(LayoutNode::Leaf(w2), LayoutNode::Leaf(w3)),
...
/// Non-home workspace should still use the old active_group logic.
/// The active window gets heavy borders even without hub state.
#[test]
fn normal_workspace_active_window() {
    let mut client = Client::for_test(Config::default());
    let w1 = new_id();
    let w2 = new_id();
    let t1 = new_id();
    ...
    let output = render_to_styled_string(&mut client, COLS, ROWS);
    insta::assert_snapshot!("normal_ws_active_window", output);
}

/// Non-home workspace with workspace bar focused.
/// All window borders should be inactive.
#[test]
fn normal_workspace_bar_focused() {
    let mut client = Client::for_test(Config::default());
    let w1 = new_id();
    let w2 = new_id();
    let t1 = new_id();
...
/// Home workspace: four widgets in a grid, bottom-right focused.
#[test]
fn home_four_widgets_bottom_right_focused() {
    let mut client = Client::for_test(Config::default());
    let (w1, w2, w3, w4) = (new_id(), new_id(), new_id(), new_id());
    let (t1, t2, t3, t4) = (new_id(), new_id(), new_id(), new_id());
    ...
    let ws = home_workspace(
        vec![
            ...
            widget_window(w2, t2, HubWidget::ChangedFiles),
            widget_window(w3, t3, HubWidget::Branches),
            widget_window(w4, t4, HubWidget::Stashes),
        ],
        hsplit(
            vsplit(LayoutNode::Leaf(w1), LayoutNode::Leaf(w2)),
            vsplit(LayoutNode::Leaf(w3), LayoutNode::Leaf(w4)),
...
/// Home workspace: switch focus from widget back to sidebar.
/// This was the original bug — the last-focused widget kept its active border.
#[test]
fn home_focus_returned_to_sidebar() {
    let mut client = Client::for_test(Config::default());
    let (w1, w2) = (new_id(), new_id());
    let (t1, t2) = (new_id(), new_id());

    let mut ws = home_workspace(
        vec![
            widget_window(w1, t1, HubWidget::RecentCommits),
            widget_window(w2, t2, HubWidget::Branches),
        ],
    ...
    // active_group still points to w1 (the last focused widget)
    ws.active_group = w1;
    ...
    insta::assert_snapshot!("home_focus_returned_to_sidebar", output);
}

/// Home workspace with multiple workspaces — verify workspace bar renders correctly.
#[test]
fn home_with_other_workspaces() {
    let mut client = Client::for_test(Config::default());
    let w1 = new_id();
    let t1 = new_id();

    let mut ws = home_workspace(
        vec![widget_window(w1, t1, HubWidget::RecentCommits)],
        LayoutNode::Leaf(w1),
    );
    ws.is_home = true;
    ...
    let output = render_to_styled_string(&mut client, COLS, ROWS);
    insta::assert_snapshot!("home_with_other_workspaces", output);
}

/// Home workspace at small terminal size — sidebar may be hidden.
#[test]
fn home_small_terminal() {
    let mut client = Client::for_test(Config::default());
    let w1 = new_id();
    let t1 = new_id();
