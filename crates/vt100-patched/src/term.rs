...
impl Attrs {
    ...
    pub fn underline(mut self, underline: bool) -> Self {
        self.underline_style = Some(if underline {
    ...
    fn write_buf(&self, buf: &mut Vec<u8>) {
        ...
        if let Some(style) = self.underline_style {
            match style {
                ...
                crate::attrs::UnderlineStyle::Double => write_param!(21),
                crate::attrs::UnderlineStyle::Curly => {
                    // SGR 4:3 — use colon sub-parameter syntax
                    if first { first = false; } else { buf.push(b';'); }
                    buf.extend_from_slice(b"4:3");
                }
                crate::attrs::UnderlineStyle::Dotted => {
                    if first { first = false; } else { buf.push(b';'); }
                    buf.extend_from_slice(b"4:4");
                }
                crate::attrs::UnderlineStyle::Dashed => {
                    if first { first = false; } else { buf.push(b';'); }
                    buf.extend_from_slice(b"4:5");
                }
            }
        }
        ...
        // Underline color uses a separate CSI sequence (SGR 58)
        if let Some(color) = self.underline_color {
            match color {
                ...
                crate::Color::Idx(i) => {
                    buf.extend_from_slice(b"\x1b[58;5;");
                    extend_itoa(buf, i);
                    buf.push(b'm');
                }
                crate::Color::Rgb(r, g, b) => {
                    buf.extend_from_slice(b"\x1b[58;2;");
                    extend_itoa(buf, r);
                    buf.push(b';');
                    extend_itoa(buf, g);
                    buf.push(b';');
