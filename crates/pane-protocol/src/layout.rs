...
impl LayoutNode {
    ...
    /// Resolve layout with manual folds: windows in `folded` are collapsed to fold bars.
        ...
        match self {
            LayoutNode::Leaf(id) => {
                if folded.contains(id) {
                    // Folded leaf reached through both-folded proportional split
            ...
            LayoutNode::Split {
                ...
                let first_all_folded = first.all_folded(folded);
                let second_all_folded = second.all_folded(folded);
                ...
                if first_all_folded && !second_all_folded {
                ...
                } else if second_all_folded && !first_all_folded {
                    ...
                    // Both visible or both folded — proportional split
                    let (first_rect, second_rect) =
                        Self::split_rects(direction, *ratio, area);
    ...
    /// Check if all leaves in this subtree are in the folded set.
    fn all_folded(&self, folded: &HashSet<TabId>) -> bool {
        match self {
            LayoutNode::Leaf(id) => folded.contains(id),
            LayoutNode::Split { first, second, .. } => {
                first.all_folded(folded) && second.all_folded(folded)
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_resolve_with_folds_empty_matches_resolve() {
        let id1 = TabId::new_v4();
        let id2 = TabId::new_v4();
        let node = LayoutNode::Split {
            direction: SplitDirection::Horizontal,
        ...
        let resolved = node.resolve_with_folds(area, &HashSet::new());
        assert_eq!(resolved.len(), 2);
    ...
    fn test_fold_second_pane() {
        ...
        // First should get most of the space
        if let ResolvedPane::Visible { rect, .. } = &resolved[0] {
            assert!(rect.width > 190);
        }
    }

    #[test]
    fn test_fold_first_pane() {
        let id1 = TabId::new_v4();
        let id2 = TabId::new_v4();
        let node = LayoutNode::Split {
            direction: SplitDirection::Horizontal,
        ...
        let area = Rect::new(0, 0, 200, 50);
        let folded: HashSet<TabId> = [id1].into_iter().collect();
        let resolved = node.resolve_with_folds(area, &folded);
    ...
    #[test]
    fn test_fold_nested_subtree_manual() {
        // H[id1 | H[id2 | id3]], fold both id2 and id3
        let id1 = TabId::new_v4();
        let id2 = TabId::new_v4();
        let id3 = TabId::new_v4();
        let node = LayoutNode::Split {
        ...
        let area = Rect::new(0, 0, 200, 50);
        let folded: HashSet<TabId> = [id2, id3].into_iter().collect();
        let resolved = node.resolve_with_folds(area, &folded);
        assert!(matches!(resolved[0], ResolvedPane::Visible { id, .. } if id == id1));
    ...
    #[test]
    fn test_fold_partial_subtree() {
        // H[id1 | H[id2 | id3]], fold only id2
        let id1 = TabId::new_v4();
        let id2 = TabId::new_v4();
        let id3 = TabId::new_v4();
        let node = LayoutNode::Split {
        ...
        assert!(resolved.iter().any(|rp| matches!(rp, ResolvedPane::Folded { id, .. } if *id == id2)));
    }

    #[test]
    fn test_all_folded_helper() {
        let id1 = TabId::new_v4();
        let id2 = TabId::new_v4();
        let node = LayoutNode::Split {
            direction: SplitDirection::Horizontal,
        ...
        assert!(!node.all_folded(&HashSet::new()));
        assert!(!node.all_folded(&[id1].into_iter().collect()));
        assert!(node.all_folded(&[id1, id2].into_iter().collect()));
    }
